...
contract Marketplace is Ownable, ReentrancyGuard {
    ...
    address public MomentiblesNFTAddr;

    uint256 public cost1 = 0.0005 ether;
    uint256 public cost2 = 0.00125 ether;
    uint256 public cost3 = 0.0025 ether;
    uint256 public cost4 = 0.005 ether;
    ...
    function setMoment(address addr) public onlyOwner {
        MomentiblesNFTAddr = addr;
    }
    ...
    function mintMarketItem(
        ...
        Momentibles(MomentiblesNFTAddr).setApprovalForAll(MomentiblesNFTAddr, true);
    }
...
function mintMarketItemToList(
    ...
    function gettokenURI(uint256 _id) public view returns(string memory) {
        string memory result_str = idToMarketItem[_id].minted ? Momentibles(MomentiblesNFTAddr).tokenURI(_id) : "";
        return result_str;
    }

    function ownerOf(uint256 _id) public view returns(address){
        address addr = idToMarketItem[_id].minted ? Momentibles(MomentiblesNFTAddr).ownerOf(_id) : address(0);
        return addr;
    }

    function balanceOf(address _addr) public view returns(uint256){
        return Momentibles(MomentiblesNFTAddr).balanceOf(_addr);
    }
    ...
    function purchaseItem(uint256 _id) public payable {
        ...
        IERC721(MomentiblesNFTAddr).transferFrom(idToMarketItem[_id].holder, msg.sender, _id);
